...
def heroes():
    ...
    for i in range(50):
        time.sleep(.1)
        sys.stdout.write('\r \033[1;36m JUBA GOOD  '+anim[i % len(anim)])
...
def menu(my_name,my_id):
	...
	print('\033[1;32mTelegram ᯼ @B_B_B_YU')
	print('')
	print('[❖ - https://t.me/J_O_P_A_4]')
	print('')
	print(f'\033[1;32m»  Your ID : '+str(my_id))
	print('')
	print(f'\033[1;32m»  Your IP : {ip}')
	print('')
	print('\033[1;32m» 1- Crack File : مــن مــلــف  ')		
	print('')
	print('')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s <JUBA> %s/%s » [OK] %s » [CP] %s » %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
